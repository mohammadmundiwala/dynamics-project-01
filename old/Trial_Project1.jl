...
# ╔═╡ 2cbdd2a9-6e3b-481a-84e1-f23f47a90a39
using Symbolics, ModelingToolkit, DifferentialEquations, Plots

# ╔═╡ 717dc89e-1457-11f1-a205-0dc95d9ebc11
...
# ╔═╡ 882975a8-a6ad-4400-97fa-01c21319af24
import Latexify

# ╔═╡ d639d158-053a-43f3-9607-03783e226dfc
default(fontfamily = "Computer Modern", linewidth = 2, size = (800, 600))

# ╔═╡ 9626b7f2-d082-4ff9-856e-f2aa3de519dc
########## CHANGE TO CODE #############

# ╔═╡ 69795f6c-97a1-497a-b279-216c5b6def1c
# definining variables and basic equations.
begin
	@parameters L h1 w1 m g Ω I_frame #***** added I_frame variable for moment of inertia 
	@independent_variables t
	@variables θ(t) φ(t) #*****making this a dynamic variable******
	D = Differential(t)
	θ_dot = D(θ)
	θ_dot_dot = D(D(θ))
    φ_dot = D(φ)                       # *********Added
    φ_dot_dot = D(D(φ))                # *********Added

end;

# ╔═╡ 97cbb147-1d33-43a5-86ac-2a546a4c18fa
r_rot = [L*sin(θ)*cos(φ), L*sin(θ)*sin(φ), -L*cos(θ)]

# ╔═╡ 415b853a-b21c-403b-9a7c-931c2a29ae91
r_0 = [w1*cos(φ), w1*sin(φ), h1]

# ╔═╡ c8c82939-6c53-4c77-9745-f8d2aac3818f
r = r_rot .+ r_0

# ╔═╡ e041e987-e7c3-43fa-b4aa-cac203567a86
# defining lagrangian
begin
	h = r[3]
	v = D.(r)
	T = m/2*sum(v .* v)  #****** pendelum kinetic energy
	T_frame = 0.5 * I_frame * φ_dot^2 ####**** added frame rotationa energy 
	V = m*g*h
	Lag = expand_derivatives(T + T_frame - V) ###### ****** added T_frame
end

# ╔═╡ 4777d7ba-289e-49e5-b818-5e5cd5d98143
# performing euler-lagrange equation
begin
    # θ equation
    dL_dθ = Symbolics.derivative(Lag, θ)
    dL_dθ_dot = Symbolics.derivative(Lag, θ_dot)
    el_eq_theta = expand_derivatives(D(dL_dθ_dot) - dL_dθ)

    # φ equation *************Addded
    dL_dφ = Symbolics.derivative(Lag, φ)
    dL_dφ_dot = Symbolics.derivative(Lag, φ_dot)
    el_eq_phi = expand_derivatives(D(dL_dφ_dot) - dL_dφ)

    # Solve for accelerations
    sol = simplify(solve_for([el_eq_theta ~ 0, el_eq_phi ~ 0], [θ_dot_dot, φ_dot_dot]))  # <-- ********* Added φ equation
end;

# ╔═╡ bc2b6a6c-105b-4251-a1dc-e3d2108c3fab
# setting up numerical solver
begin
    @variables x1(t) x2(t) x3(t) x4(t)  # x1=θ, x2=θ̇, x3=φ, x4=φ̇

    # Substitute into solution
    rhs_exprs = substitute(sol, Dict(θ => x1, θ_dot => x2, φ => x3, φ_dot => x4))

    eqs = [
        D(x1) ~ x2,                     # θ̇
        D(x2) ~ rhs_exprs[θ_dot_dot],   # θ̈
        D(x3) ~ x4,                     # φ̇
        D(x4) ~ rhs_exprs[φ_dot_dot]    # φ̈ <-- Added
    ]

    @named sys = ODESystem(eqs, t, [x1, x2, x3, x4], [L, g, m, w1, h1, I_frame])  # Added x3, x4 and I_frame
    sys = structural_simplify(sys)
end

# ╔═╡ 56cfa811-ab3b-4c23-a848-1cd8417a2b20
begin
    u0 = [
        x1 => 0.5,  # θ
        x2 => 0.0,  # θ̇
        x3 => 0.0,  # φ
        x4 => 2.0   # initial φ̇ instead of Ω <-- Added
    ]

    tspan = (0.0, 13.0)
    p_vals = Dict(L => 0.15, g => 9.8, m => 0.1, w1 => 0.1, h1 => 0.2, I_frame => 0.002)  # <-- Added I_frame

    prob = ODEProblem(sys, u0, tspan, p_vals)
    solution = solve(prob, RK4(); reltol=1e-6, abstol=1e-8, saveat=0.0333)
end;

# ╔═╡ f686583f-b51b-427c-ba48-9ee28b6153cd


# ╔═╡ 31865c32-bd47-47b1-9c9d-d3c82f989b7c
Plots.plot(solution, idxs=[x1], xlabel="t", ylabel="θ (rad)", title="Angle vs. Time")
Plots.plot(solution, idxs=[x2], xlabel="t", ylabel="θ̇ (rad/s)", title="Angular Velocity vs. Time")
Plots.plot(solution, idxs=[x3], xlabel="t", ylabel="φ (rad)", title="Frame Rotation")
Plots.plot(solution, idxs=[x4], xlabel="t", ylabel="φ̇ (rad/s)", title="Frame Rotation Speed")

# ╔═╡ 4d1b2ef6-1ae5-4a29-9cc7-4f583c4daa7a
begin
    θ_vals = solution[x1]
    φ_vals = solution[x3]   # <-- Changed from Ω*t_vals
    t_vals = solution.t
...
# ╔═╡ 5071a857-d210-4e8a-a791-6158c34180f0
begin
...
# ╔═╡ d3a789cc-ef8c-4143-9637-63b3019369a3
...
manifest_format = "2.0"
project_hash = "52f003bc8a240a93341dabdfc04dd927e9f87088"
